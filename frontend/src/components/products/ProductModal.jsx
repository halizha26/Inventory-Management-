...
const defaultCategoryOptions = Object.keys(CATEGORY_TREE).map(cat => ({
  value: cat,
  label: cat
}));
...
const ProductModal = ({ isOpen, onClose, productToEdit, onSuccess, products = [] }) => {
  ...
  // 👇 DITAMBAHKAN 'watch' DI SINI 👇
  const { register, handleSubmit, reset, setValue, watch, formState: { errors, isSubmitting } } = useForm({
    resolver: yupResolver(schema),
    defaultValues: { currency: 'IDR' },
  });

  // 👇 PASANG ROBOT MATA-MATA UNTUK KALKULASI LIVE TOTAL 👇
  ...
  useEffect(() => {
    if (Array.isArray(products) && products.length > 0) {
      const existingCategories = products.map(p => {
        return p?.category ? p.category.split(' - ')[0] : '';
      }).filter(Boolean);
  ...
  const onSubmit = async (data) => {
    try {
      if (isLearningMaterial) {
        if (!subKategori || !detailKategori) { toast.error('Mohon lengkapi Merek dan Modul Barang!'); return; }
        if (hasVarian && !varianKategori) { toast.error('Modul ini memiliki Varian, mohon dipilih!'); return; }
      }
              ...
              <form onSubmit={handleSubmit(onSubmit)} className="p-8 space-y-8 bg-slate-50/50">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div className="flex flex-col gap-1">
                      ...
                      <div className="flex border-2 border-slate-200 rounded-xl overflow-hidden focus-within:ring-4 focus-within:ring-emerald-100 focus-within:border-emerald-500 transition-all bg-white">
                        <select className="px-4 py-4 bg-slate-50 border-r-2 border-slate-200 font-black text-emerald-600 text-sm outline-none cursor-pointer" {...register('currency')}>
                          {CURRENCIES.map(c => <option key={c} value={c}>{c === 'IDR' ? 'Rp' : '$'}</option>)}
                        </select>
                        <input type="number" step="0.01" placeholder="0" className="w-full px-4 py-4 font-black text-xl text-emerald-700 outline-none" {...register('price')} />
                      </div>
                      {errors.price && <p className="text-[10px] font-black text-red-600 mt-1 uppercase">{errors.price.message}</p>}
                    </div>
                  </div>

                  {/* 👇 TERBARU: PREVIEW TOTAL BIAYA LIVE DI DALAM MODAL FORM 👇 */}
                ...
                <div className="flex flex-col gap-1 bg-white p-6 rounded-[24px] border-2 border-slate-200 shadow-sm">
                  ...
                  {supplierType === 'regular' ? (
                    <select className={`w-full px-4 py-4 border-2 rounded-xl bg-white focus:ring-4 focus:ring-orange-100 focus:border-orange-500 font-bold text-sm outline-none transition-all ${errors.supplier ? 'border-red-400' : 'border-slate-200'}`} {...register('supplier')}>
